...
def generate_launch_description():
    package_name = 'rm_eco65_gripper'

    robot_name_in_model = 'rm_eco65_gripper_description'

    pkg_share = FindPackageShare(package=package_name).find(package_name) 
    urdf_model_path = os.path.join(pkg_share, f'urdf/rm_eco65_gripper_description.urdf.xacro')
    ...
    load_joint_trajectory_controller1 = ExecuteProcess(
        cmd=['ros2', 'control', 'load_controller', '--set-state', 'active',
    ...
    close_evt2 = RegisterEventHandler(
            event_handler=OnProcessExit(
                ...
                on_exit=[load_joint_trajectory_controller1],
            )
    )

    close_evt3 = RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=load_joint_trajectory_controller1,
